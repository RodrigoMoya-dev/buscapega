# Integración n8n — Buscapega

## Qué se hizo

Se instaló y configuró n8n en el servidor **Presto** como orquestador del sistema Buscapega. n8n actúa como el cerebro de la automatización: programa cuándo se buscan ofertas, recibe los resultados de postulación y envía notificaciones por Telegram al usuario.
...
| **Backend (FastAPI)** | `presto:8020` | API principal de Buscapega |
...
Todos los servicios de Buscapega se gestionan desde:
```
~/docker/buscapega/docker-compose.yml
```
...
En el menú lateral izquierdo, bajo **Workflows**, aparecen los 3 flujos de Buscapega. Haciendo click en cada uno se abre el editor visual donde se pueden ver los nodos, sus conexiones, y el historial de ejecuciones.
...
```
[Webhook: POST /webhook/buscapega-apply-result]
    │
    ▼
[IF: ¿status == "ok"?]
    │
...
```
http://localhost:5678/webhook/buscapega-apply-result
```
...
| Dato | Valor |
|---|---|
| Nombre del bot | @Buscapega_bot |
| Chat ID del usuario | `8301524530` |
| Token | guardado en `~/docker/buscapega/.env` |
...
En `~/docker/buscapega/.env`:

```env
ANTHROPIC_API_KEY=...          # Claude API para evaluación y cartas
TELEGRAM_BOT_TOKEN=...         # Token del bot @Buscapega_bot
...
```env
N8N_WEBHOOK_URL=http://localhost:5678/webhook/buscapega-apply-result
```
...
- **Local (Mac):** `buscapega/setup/cookies/<portal>_session.json`
- **Presto:** `~/docker/buscapega/cookies/<portal>_session.json`

Para renovar o crear una sesión:
```bash
cd "/Users/rodrigo/Proyectos/Moya.dev/Proyectos internos/buscapega/setup"
...
cd ~/docker/buscapega && docker compose ps

# Ver logs en tiempo real
docker logs buscapega_backend -f
docker logs buscapega_scraper -f
...
curl -X POST http://localhost:5678/webhook/buscapega-apply-result \
